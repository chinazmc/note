...
书上说，处于 [TIME_WAIT](https://so.csdn.net/so/search?q=TIME_WAIT&spm=1001.2101.3001.7020) 状态的连接，在收到相同四元组的 SYN 后，会回 RST 报文，对方收到后就会断开连接。
...
针对这个问题，**关键是要看 SYN 的「序列号和时间戳」是否合法**，因为处于 TIME_WAIT 状态的连接收到 [SYN](https://so.csdn.net/so/search?q=SYN&spm=1001.2101.3001.7020) 后，会判断 SYN 的「序列号和时间戳」是否合法，然后根据判断结果的不同做不同的处理。
...
> PS：这里先埋一个疑问，处于 TIME_WAIT 状态的连接，收到 [RST](https://so.csdn.net/so/search?q=RST&spm=1001.2101.3001.7020) 会断开连接吗？
...
# Reference
https://blog.csdn.net/qq_34827674/article/details/123227502