...
![[Pasted image 20230612174517.png]]
...
这时候，网络就变成了这个样子:
![[Pasted image 20230612174417.png]]
...
于是网络变成了这样子：
![[Pasted image 20230612175811.png]]
...
![[Pasted image 20230612180104.png]]
...
- 上面虽然通了，但由于[Linux下arp的特性](https://link.segmentfault.com/?enc=Uv7F3sfoez%2BylxvVAbn%2FDw%3D%3D.RnJIIKi19mk7afAZntqVd7zVjHAVD6KKM1xrDCEbD1Y%3D)，当协议栈收到外面的arp请求时，不管是问101还是102，都会回复两个arp应答，分别包含br0和veth1的mac地址，也即Linux觉得外面发给101和102的数据包从br0和veth1进协议栈都一样，没有区别。由于回复了两个arp应答，而外面的设备只会用其中的一个，并且具体用哪个会随着时间发生变化，于是导致一个问题，就是外面回复给102的数据包可能从101的br0上进来，即通过102 ping外面时，可能在veth1抓不到回复包，而在br0上能抓到回复包。说明数据流在交换机那层没有完全的隔离开，br0和veth1会收到对方的IP应答包。为了解决上述问题，可以配置rp_filter, arp_filter, arp_ignore, arp_announce等参数，但不建议这么做，容易出错，调试比较麻烦。
...
![[Pasted image 20230612180319.png]]
