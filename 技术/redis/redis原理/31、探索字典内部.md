...
![[Pasted image 20230530153526.png]]
...
```
![[Pasted image 20230530153737.png]]
...
不用考虑bgsave**主要是因为我们的缩容的内存都是已经使用过的，缩容的时候可以直接置空，而且由于申请的内存比较小，同时会释放掉一些已经使用的内存，不会增大系统的压力**。

扩容考虑bgsave是因为bgsave是采用COW方式和父进程共享内存，如果此时发生rehash，会导致大量的页面写操作从而触发缺页异常，导致大量页面加载，会引发抖动。 但是如果 hash 表元素个数已经达到了第一维数组5倍(dict_force_resize_ratio)，会强制扩容。
所以一般在bgsave期间会把负载因子提升，避免rehash。