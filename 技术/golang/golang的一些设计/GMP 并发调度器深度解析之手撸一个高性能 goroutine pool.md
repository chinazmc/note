...
这些问题实在太扎眼了，导致 Go1.0 虽然号称原生支持并发，却在并发性能上一直饱受诟病，然后，Go 语言委员会中一个核心开发大佬看不下了，亲自下场重新设计和实现了 Go 调度器（在原有的 G-M 模型中引入了 P）并且实现了一个叫做 [_work-stealing_](https://supertech.csail.mit.edu/papers/steal.pdf) 的调度算法：
...
_**至此，Go 调度器的基本模型确立：**_

![](https://img.taohuawu.club/gallery/GMP-scheduler.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
```go
 1func (srv *Server) ListenAndServe() error { 2	addr := srv.Addr 3	if addr == "" { 4		addr = ":http" 5	} 6	ln, err := net.Listen("tcp", addr) 7	if err != nil { 8		return err 9	}10	return srv.Serve(tcpKeepAliveListener{ln.(*net.TCPListener)})11}
```

看到最后那个 srv.Serve 调用了吗？没错，这个 `Serve` 方法里面就是实际处理 http 请求的逻辑，我们再进入这个方法内部：

```go
 1func (srv *Server) Serve(l net.Listener) error { 2	defer l.Close() 3	... 4    // 不断循环取出TCP连接 5	for { 6        // 看我看我！！！ 7		rw, e := l.Accept() 8        ... 9        // 再看我再看我！！！10		go c.serve(ctx)11	}12}
```
...
![](https://pic1.zhimg.com/80/v2-5d976868546cbab2b8462e5bf06398cc_1440w.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![](https://img.taohuawu.club/gallery/66396519-7ed66e00-ea0c-11e9-9c1a-5ca54bbd61eb.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
```go
 1type sig struct{} 2 3type f func() error 4 5// Pool accept the tasks from client,it limits the total 6// of goroutines to a given number by recycling goroutines. 7type Pool struct { 8	// capacity of the pool. 9	capacity int321011	// running is the number of the currently running goroutines.12	running int321314	// expiryDuration set the expired time (second) of every worker.15	expiryDuration time.Duration1617	// workers is a slice that store the available workers.18	workers []*Worker1920	// release is used to notice the pool to closed itself.21	release chan sig2223	// lock for synchronous operation.24	lock sync.Mutex2526	once sync.Once27}
```
...
```go
 1// NewPool generates a instance of ants pool 2func NewPool(size int) (*Pool, error) { 3	return NewTimingPool(size, DefaultCleanIntervalTime) 4} 5 6// NewTimingPool generates a instance of ants pool with a custom timed task 7func NewTimingPool(size, expiry int) (*Pool, error) { 8	if size <= 0 { 9		return nil, ErrInvalidPoolSize10	}11	if expiry <= 0 {12		return nil, ErrInvalidPoolExpiry13	}14	p := &Pool{15		capacity:       int32(size),16		freeSignal:     make(chan sig, math.MaxInt32),17		release:        make(chan sig, 1),18		expiryDuration: time.Duration(expiry) * time.Second,19	}20	// 启动定期清理过期worker任务，独立goroutine运行，21	// 进一步节省系统资源22	p.monitorAndClear()23	return p, nil24}
```
...
```go
1// Submit submit a task to pool2func (p *Pool) Submit(task f) error {3	if len(p.release) > 0 {4		return ErrPoolClosed5	}6	w := p.getWorker()7	w.task <- task8	return nil9}
```
...
```go
 1// getWorker returns a available worker to run the tasks. 2func (p *Pool) getWorker() *Worker { 3	var w *Worker 4	// 标志变量，判断当前正在运行的worker数量是否已到达Pool的容量上限 5	waiting := false 6	// 加锁，检测队列中是否有可用worker，并进行相应操作 7	p.lock.Lock() 8	idleWorkers := p.workers 9	n := len(idleWorkers) - 110	// 当前队列中无可用worker11	if n < 0 {12		// 判断运行worker数目已达到该Pool的容量上限，置等待标志13		waiting = p.Running() >= p.Cap()1415		// 当前队列有可用worker，从队列尾部取出一个使用16	} else {17		w = idleWorkers[n]18		idleWorkers[n] = nil19		p.workers = idleWorkers[:n]20	}21	// 检测完成，解锁22	p.lock.Unlock()23	// Pool容量已满，新请求等待24	if waiting {25		// 利用锁阻塞等待直到有空闲worker26		for {27			p.lock.Lock()28			idleWorkers = p.workers29			l := len(idleWorkers) - 130			if l < 0 {31				p.lock.Unlock()32				continue33			}34			w = idleWorkers[l]35			idleWorkers[l] = nil36			p.workers = idleWorkers[:l]37			p.lock.Unlock()38			break39		}40		// 当前无空闲worker但是Pool还没有满，41		// 则可以直接新开一个worker执行任务42	} else if w == nil {43		w = &Worker{44			pool: p,45			task: make(chan f, 1),46		}47		w.run()48		// 运行worker数加一49		p.incRunning()50	}51	return w52}
```
...
```go
 1// Worker is the actual executor who runs the tasks, 2// it starts a goroutine that accepts tasks and 3// performs function calls. 4type Worker struct { 5	// pool who owns this worker. 6	pool *Pool 7 8	// task is a job should be done. 9	task chan f1011	// recycleTime will be update when putting a worker back into queue.12	recycleTime time.Time13}1415// run starts a goroutine to repeat the process16// that performs the function calls.17func (w *Worker) run() {18	go func() {19		// 循环监听任务列表，一旦有任务立马取出运行20		for f := range w.task {21			if f == nil {22				// 退出goroutine，运行worker数减一23				w.pool.decRunning()24				return25			}26			f()27			// worker回收复用28			w.pool.putWorker(w)29		}30	}()31}
```
...
```go
1// putWorker puts a worker back into free pool, recycling the goroutines.2func (p *Pool) putWorker(worker *Worker) {3	// 写入回收时间，亦即该worker的最后一次结束运行的时间4	worker.recycleTime = time.Now()5	p.lock.Lock()6	p.workers = append(p.workers, worker)7	p.lock.Unlock()8}
```

### 动态扩容或者缩小池容量

```go
 1// ReSize change the capacity of this pool 2func (p *Pool) ReSize(size int) { 3	if size == p.Cap() { 4		return 5	} 6	atomic.StoreInt32(&p.capacity, int32(size)) 7	diff := p.Running() - size 8	if diff > 0 { 9		for i := 0; i < diff; i++ {10			p.getWorker().task <- nil11		}12	}13}
```

### 定期清理过期 Worker

```go
 1// clear expired workers periodically. 2func (p *Pool) periodicallyPurge() { 3	heartbeat := time.NewTicker(p.expiryDuration) 4	for range heartbeat.C { 5		currentTime := time.Now() 6		p.lock.Lock() 7		idleWorkers := p.workers 8		if len(idleWorkers) == 0 && p.Running() == 0 && len(p.release) > 0 { 9			p.lock.Unlock()10			return11		}12		n := 013		for i, w := range idleWorkers {14			if currentTime.Sub(w.recycleTime) <= p.expiryDuration {15				break16			}17			n = i18			w.task <- nil19			idleWorkers[i] = nil20		}21		n++22		if n >= len(idleWorkers) {23			p.workers = idleWorkers[:0]24		} else {25			p.workers = idleWorkers[n:]26		}27		p.lock.Unlock()28	}29}
```
...
![pool1.png](https://raw.githubusercontent.com/panjf2000/illustrations/master/go/ants-pool-1.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)

![pool2.png](https://raw.githubusercontent.com/panjf2000/illustrations/master/go/ants-pool-2.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)

![pool3.png](https://raw.githubusercontent.com/panjf2000/illustrations/master/go/ants-pool-3.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)

![pool4.png](https://raw.githubusercontent.com/panjf2000/illustrations/master/go/ants-pool-4.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
```go
 1type pf func(interface{}) error 2 3// PoolWithFunc accept the tasks from client,it limits the total 4// of goroutines to a given number by recycling goroutines. 5type PoolWithFunc struct { 6	// capacity of the pool. 7	capacity int32 8 9	// running is the number of the currently running goroutines.10	running int321112	// expiryDuration set the expired time (second) of every worker.13	expiryDuration time.Duration1415	// workers is a slice that store the available workers.16	workers []*WorkerWithFunc1718	// release is used to notice the pool to closed itself.19	release chan sig2021	// lock for synchronous operation.22	lock sync.Mutex2324	// pf is the function for processing tasks.25	poolFunc pf2627	once sync.Once28}
```
...
```go
 1// WorkerWithFunc is the actual executor who runs the tasks, 2// it starts a goroutine that accepts tasks and 3// performs function calls. 4type WorkerWithFunc struct { 5	// pool who owns this worker. 6	pool *PoolWithFunc 7 8	// args is a job should be done. 9	args chan interface{}1011	// recycleTime will be update when putting a worker back into queue.12	recycleTime time.Time13}1415// run starts a goroutine to repeat the process16// that performs the function calls.17func (w *WorkerWithFunc) run() {18	go func() {19		for args := range w.args {20			if args == nil {21				w.pool.decRunning()22				return23			}24			w.pool.poolFunc(args)25			w.pool.putWorker(w)26		}27	}()28}
```
...
测试结果：  
![](https://pic3.zhimg.com/80/v2-420e2f361512a0c1c7aeddcc51c07af2_1440w.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)  
...
测试结果：  
![](https://pic2.zhimg.com/80/v2-883b94788a230d884c0390fdefe9c281_1440w.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![](https://pic1.zhimg.com/80/v2-0f834974d61d6af0fc1a11da260b58cc_1440w.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![timg.jpeg](https://img.hacpai.com/file/2020/03/timg-738964dc.jpeg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![](https://pic3.zhimg.com/80/v2-a6258eda1a008b5294e9ebb11b697eca_1440w.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)

### 100w 吞吐量

![](https://pic4.zhimg.com/80/v2-081e117453f840decb1cbb16a28c1893_1440w.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)

### 1000w 吞吐量

![](https://pic3.zhimg.com/80/v2-5976a6688c69d29be1b22dbb70e7d7b6_1440w.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
